    ...
    def collect_all(self, force_update: bool = False) -> dict:
        ...
        for url in self.urls:
            ...
            try:
                ...
                # Guard: reject bot-detection pages (Cloudflare challenge etc.)
                # that return HTTP 200 but contain no fund data.
                if b"__NEXT_DATA__" not in content:
                    logger.warning(
                        f"'{slug}' response is missing __NEXT_DATA__ — "
                        "likely a bot-detection page. Preserving existing file."
                    )
                    results[slug] = {"status": "skipped_invalid", "path": str(file_path)}
                    continue
